...
function App() {
  // STATES TO GET AND UPDATE TIMER
  const [countDay, setCountDay] = useState(0);
  const [countHour, setCountHour] = useState(0);
  const [countMin, setCountMin] = useState(0);
  const [countSec, setCountSec] = useState(0);

  // FUNCTION TO CALCULATE A COUNT DOWN
  const count = () => {
    const countDate = new Date('October 12,2023 00:00:00').getTime();
    const currentDate = new Date().getTime();
    const totalDate = countDate - currentDate;
    ...
    const days = Math.floor(totalDate / day);
    const hours = Math.floor((totalDate % day) / hour);
    const min = Math.floor((totalDate % hour) / minute);
    const sec = Math.floor((totalDate % minute) / second);
